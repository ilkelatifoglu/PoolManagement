...
const ManageMembership = ({ pools, onMembershipChange, managerId }) => {
  ...
  const [newMembership, setNewMembership] = useState({ price: "", duration: "", pool_id: "" });
    ...
    <div>
      ...
      <form onSubmit={handleCreateMembership} style={{ marginBottom: "20px" }}>
        <select
          ...
          onChange={(e) => setNewMembership({ ...newMembership, pool_id: e.target.value })}
          required
        >
      ...
      {memberships.length > 0 ? (
        memberships.map((membership) => (
          <tr key={membership.membership_id}>
            ...
            <td>
              <button
                onClick={() => handleDeleteMembership(membership.membership_id)}
